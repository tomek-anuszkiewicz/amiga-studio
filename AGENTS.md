...
2. **Clock & Execution Model (Color Clock Phases)**:
   ...
   - All memory bus operations must respect bus readiness (memory access methods `read_byte`, `read_word`, `write_byte`, `write_word` return `BusResult::WaitState` when Chip RAM is blocked by custom chip DMA). If Chip RAM is blocked, the CPU waits additional CCK cycles (accumulating wait states, measurable via elapsed `cycle_counter`) without advancing its active micro-step.
...
7. **Module Cohesion & Rust Source File Size Guidelines (`.rs` Files Only)**:
   ...
   - **Strict Flat Instruction Hierarchy & 1:1 Mnemonic Mapping (`crates/m68000/src/instructions/`)**:
     ...
     - **Prohibition of Umbrella/Grouping Files**: Grouping disparate instruction mnemonics into umbrella files (such as `mul.rs`, `div.rs`, `link_unlk.rs`, `bcd.rs`, or `privileged.rs`) is strictly forbidden and actively validated by automated architecture tests.
     - **Zero Subdirectories in `instructions/`**: Creating subdirectories or multi-file submodules under `crates/m68000/src/instructions/` (such as `and/`, `cmpi/`, `or/`, or `div/`) is **strictly forbidden**. The directory must remain strictly flat at all times; any subdirectory at any depth triggers an immediate architecture test failure.
     - **Recognized Exceptions**:
       - Tightly coupled Condition Code Register / Status Register operations sharing identical execution mechanics: `move_sr_ccr.rs` (`MOVE from/to SR/CCR`) and `logic_sr_ccr.rs` (`ANDI/EORI/ORI to CCR/SR`).
       - Size-based decompositions for high-cardinality operations with exhaustive addressing mode matrices (`move_b.rs`, `move_w.rs`, `move_l.rs`).
     - When an exhaustive instruction decoder or micro-step table exceeds 800 lines due to numerous addressing mode variations (e.g. `add.rs`, `sub.rs`, `and.rs`, `or.rs`, `cmpi.rs`, `move_b.rs`), it qualifies as an authorized **Recognized Exception** under the rule below and must be registered in `LINE_COUNT_EXCEPTIONS` in `test_architecture_rules.rs`. It must NEVER be split into subdirectories.
