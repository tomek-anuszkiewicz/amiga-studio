...
2. Stage `01b_ocr` inspects the resulting `page_XXXX.json` files:
   ...
   - **Scanned Pages / Covers:** If a page contains 0 blocks or empty text, Stage 01b automatically activates **Gemini Vision OCR** on `page_XXXX.png` to extract paragraphs, headings, and tables with normalized bounding boxes (`bbox_norm`).
