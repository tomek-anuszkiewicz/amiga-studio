...
- `header`: Running top-margin document header, section continuation title, or chapter title rule at the very top of the page. This includes running page headers on continuation pages such as 'LIST OF ILLUSTRATIONS (Continued)', 'LIST OF ILLUSTRATIONS (Concluded)', 'LIST OF TABLES (Continued)', 'LIST OF TABLES (Concluded)', 'TABLE OF CONTENTS (Continued)', or repeated chapter/section titles along the top margin. These top-margin continuation headers MUST be classified as `header` (NOT as `toc_header`, `heading`, or `caption_continuation`).
- `footer`: Running bottom-margin footer or page number at the very bottom of the page.
- `toc_header`: Prominent Table of Contents opening title banner (e.g. 'Contents', 'Table of Contents'). This is strictly for the primary Table of Contents opening title. NEVER classify running top-margin headers on continuation pages (such as 'LIST OF TABLES (Concluded)', 'LIST OF TABLES (Continued)', 'LIST OF ILLUSTRATIONS (Concluded)', 'LIST OF ILLUSTRATIONS (Continued)', or 'TABLE OF CONTENTS (Continued)') as `toc_header`—they are running document `header`s. Furthermore, standalone titles for lists of illustrations or tables (e.g. 'LIST OF ILLUSTRATIONS', 'LIST OF TABLES') must NOT be classified as `toc_header`—if positioned as a running header at the top margin, classify them as `header`; if introducing the section in the body, classify them as `heading`.
...
- `heading`: Section headings, subheadings, topic titles, and data block titles within an ongoing chapter (e.g. 'Copper Instruction Summary', 'Register Map', '256 Byte Sample', '128 Byte Sample'). A `heading` block must consist solely of the title itself. If a paragraph begins with a run-in heading followed immediately by body text sentences in the same block (e.g. '1.2.3.4 ACCRUED EXCEPTION BYTE. The AEXC byte contains five exception bits...'), you MUST classify the block as `prose` (NOT `heading`), so that the run-in title can be formatted inline as bold without turning the entire paragraph into a heading. Do NOT classify standalone subsection titles as table captions unless it is a formal table title starting with 'Table X-Y:'. Do NOT classify standalone advisory labels ('NOTE', 'WARNING', 'CAUTION', 'IMPORTANT', 'TIP') as heading—classify them as `callout`. Do NOT classify multi-column horizontal instruction header banners as heading—classify them as `table`. Use heading_level=1 for major sections, 2 for subsections, 3 for sub-headers.
- `callout`: Standalone advisory label or banner that introduces a note, warning, caution, or tip (e.g. 'NOTE', 'NOTE:', 'WARNING', 'CAUTION', 'IMPORTANT', 'TIP'). NEVER classify these advisory labels as `heading`—classify them as `callout`.
- `callout_text`: The body text blocks, paragraphs, bullet lists, or mathematical formulas that belong inside the callout (visually indented underneath a `callout` label or enclosed within an advisory box/border).
- `prose`: Standard narrative prose body paragraphs. Includes paragraphs that start with run-in subsection headings followed by body text. Do NOT classify indented or boxed advisory note paragraphs as `prose`—classify them as `callout_text`.
...
- `table`: Formal tabular data grids, multi-column register bit assignments, structured parameter lists, and multi-column instruction header banners (e.g. where the instruction mnemonic, full title, and processor models are laid out horizontally across parallel columns at the top of an instruction specification). Do NOT classify formal table caption lines (e.g. 'Table 5-8: Five Octave Even-tempered Scale') as table—classify the title line separately as caption.
