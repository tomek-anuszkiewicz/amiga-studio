...
- **Amiga Integration**: `rag_qdrant` is the canonical interface to the shared `projects_docs` collection. After documentation changes, incrementally index the `amiga` source with `rag_qdrant . --source amiga --include-dirs docs` and `rag_qdrant "Obsidian/Amiga" --source amiga --include-dirs Design Reference`. These commands reuse the SHA-256 cache. Add `--reindex` to both only for a complete scoped rebuild. The MCP server in [`tools/amiga-rag-mcp-server/`](../../tools/amiga-rag-mcp-server/) is a convenience adapter that delegates to this CLI.
