...
> - **Canonical Opcode Dispatch Table & Instruction Forwarders:** Refactored all 65,536 entries in `DISPATCH_TABLE` (`dispatch_table.rs`) to point to canonically named methods (`op_<mnemonic>_<size>_<source>_<destination>`) adhering to `.agents/rules/opcode-naming.md`. Implemented 1,163 specialized forwarder functions across all 47 instruction files (including flat `move_b.rs`, `move_w.rs`, `move_l.rs`), keeping all files strictly under 600 lines ($\le 800$ line limit) while preserving 100% green passes across all 77 SingleStepTests suites and architecture checks.
> - **Cycle-Exact Micro-Step State Machine Architecture (Complete M68000 Engine):** Full architectural transition of the M68000 CPU core to the **Micro-Step State Machine Engine** (`crates/m68000/src/micro/`). Exactly 65,536 static descriptors in `OPCODE_DESCRIPTOR_TABLE` mapping opcodes to immutable `&'static [MicroStep]` sequences, specialized atomic `MicroAction` variants, cached slice pointer execution (`current_steps`), bus-free parametric ALU function pointers (`AluFn`), immediate value timing matching on Step 0 (`FetchExtension`), Address Error prefetch pipeline advance and post-increment suppression, and complete elimination of the legacy `DISPATCH_TABLE: [OpcodeHandler; 65536]`. Zero custom macros, zero const-generic handlers, 100% of `.rs` files $\le 800$ lines, and 100% green pass rate across all 87 SingleStepTests suites (~300,000 test cases with `$env:SINGLESTEP_FULL = "1"`).
> - **Passive MemoryBus Architecture & Direct 2-Phase CCK Execution:** Refactored the memory bus and CPU micro-engine: removed artificial 2-phase scaffolding (`MemoryBusResult::Phase1Ready`, `begin_cycle`, `end_cycle`, `active_bus_cycle`, `handle_bus_cycle_completion`), made `MemoryBus` passive (`is_chip_ram_blocked`, `read_byte/word`, `write_byte/word`), and restructured `execute_micro_step` into dedicated, single-purpose helper functions (`step_bus_read_byte`, `step_bus_write_word`, etc.) across CCK1 and CCK2. Pruned all dead code (including `MicroRetireMode`), strictly maintained all source files $\le 800$ lines, and achieved 100% green pass rate across workspace unit tests, architectural compliance rules, and full SingleStepTests suite (~300,000 test cases).
