...
//! size-specialized ALU logic functions (`or_b`, `or_w`, `or_l`, `execute_or`).
...
fn exec_or_ea_to_dn(cpu: &mut Cpu, bus: &mut MemoryBus, s: u8, m: u8) -> StepResult {
    ...
    if s == SIZE_LONG {
        if m == EA_DN || m == EA_AN {
            match cpu.state.micro.micro_step {
                ...
                1 => {
                    ...
                    let res = execute_or(&mut cpu.state, s_val, d_val, Size::Long);
        ...
        let reg_d = ((cpu.state.ir >> 9) & 7) as usize;
        let d_val = cpu.state.d_long(reg_d);
        let res = execute_or(&mut cpu.state, s_val, d_val, size);
...
fn exec_or_dn_to_ea(cpu: &mut Cpu, bus: &mut MemoryBus, s: u8, m: u8) -> StepResult {
    ...
    if m == EA_DN {
        ...
        if phase == 0 {
            ...
            let reg_d = ((cpu.state.ir >> 9) & 7) as usize;
            let d_val = cpu.state.d_long(reg_d);
            let res = execute_or(&mut cpu.state, d_val, mem_val, size);
