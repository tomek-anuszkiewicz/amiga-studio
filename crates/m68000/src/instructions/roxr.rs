...
/// Evaluates ROXR rotate operation and updates condition codes (X, N, Z, V, C)
#[inline(always)]
...
/// Sized wrapper for ROXR
#[inline]
pub fn execute_roxr_sized(state: &mut CpuState, count: u32, val: u32, size: Size) -> u32 {
    let s = match size {
        Size::Byte => SIZE_BYTE,
        Size::Word => SIZE_WORD,
        Size::Long => SIZE_LONG,
    };
    execute_roxr(state, s, count, val)
}

/// Register rotate: `ROXR Dx, Dy` or `ROXR #<data>, Dy`
pub fn op_roxr_reg(
    cpu: &mut Cpu,
    _bus: &mut MemoryBus,
) -> StepResult {
    ...
    match cpu.state.micro.micro_step {
        0 => {
            ...
            let res = execute_roxr(&mut cpu.state, s, count, val);
...
/// Memory rotate: `ROXR <ea>` (word size, count = 1)
pub fn op_roxr_mem(
    cpu: &mut Cpu,
    bus: &mut MemoryBus,
) -> StepResult {
    ...
    if phase == 0 {
        ...
        let res = execute_roxr(&mut cpu.state, SIZE_WORD, 1, mem_val);
...
pub fn op_roxr_b_dn_dn(cpu: &mut Cpu, bus: &mut MemoryBus) -> StepResult {
    op_roxr_reg(cpu, bus)
}

pub fn op_roxr_b_imm_dn(cpu: &mut Cpu, bus: &mut MemoryBus) -> StepResult {
    op_roxr_reg(cpu, bus)
}

pub fn op_roxr_l_dn_dn(cpu: &mut Cpu, bus: &mut MemoryBus) -> StepResult {
    op_roxr_reg(cpu, bus)
}

pub fn op_roxr_l_imm_dn(cpu: &mut Cpu, bus: &mut MemoryBus) -> StepResult {
    op_roxr_reg(cpu, bus)
}

pub fn op_roxr_w_absl(cpu: &mut Cpu, bus: &mut MemoryBus) -> StepResult {
    op_roxr_mem(cpu, bus)
}

pub fn op_roxr_w_absw(cpu: &mut Cpu, bus: &mut MemoryBus) -> StepResult {
    op_roxr_mem(cpu, bus)
}

pub fn op_roxr_w_ai(cpu: &mut Cpu, bus: &mut MemoryBus) -> StepResult {
    op_roxr_mem(cpu, bus)
}

pub fn op_roxr_w_disp(cpu: &mut Cpu, bus: &mut MemoryBus) -> StepResult {
    op_roxr_mem(cpu, bus)
}

pub fn op_roxr_w_dn_dn(cpu: &mut Cpu, bus: &mut MemoryBus) -> StepResult {
    op_roxr_reg(cpu, bus)
}

pub fn op_roxr_w_idx(cpu: &mut Cpu, bus: &mut MemoryBus) -> StepResult {
    op_roxr_mem(cpu, bus)
}

pub fn op_roxr_w_imm_dn(cpu: &mut Cpu, bus: &mut MemoryBus) -> StepResult {
    op_roxr_reg(cpu, bus)
}

pub fn op_roxr_w_pd(cpu: &mut Cpu, bus: &mut MemoryBus) -> StepResult {
    op_roxr_mem(cpu, bus)
}

pub fn op_roxr_w_pi(cpu: &mut Cpu, bus: &mut MemoryBus) -> StepResult {
    op_roxr_mem(cpu, bus)
}
