//! ROXR (Rotate Right with Extend) instruction handlers and CCR updates
//!
...
/// Decodes the micro-step sequence for ROXR register shift
...
/// Decodes the micro-step sequence for ROXR memory shift (Word only, Count = 1)
pub const fn decode_roxr_mem_steps(mode: u8, reg: u8) -> Option<&'static [MicroStep]> {
    match mode {
        ...
        5 => Some(&STEPS_ROXR_W_D16_AN),
        6 => Some(&STEPS_ROXR_W_IDX_AN),
        7 => match reg {
