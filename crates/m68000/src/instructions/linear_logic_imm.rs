//! Cycle-exact linear M68000 immediate logic instructions (ANDI, ORI, EORI)
//!
...
pub fn op_eori(cpu: &mut Cpu, bus: &mut MemoryBus) -> StepResult {
    ...
    let s = ((ir >> 6) & 3) as u8;
    let m = decode_ea_index(((ir >> 3) & 7) as u8, (ir & 7) as u8);
    exec_eori(cpu, bus, s, m)
}

// ============================================================================
// Dedicated Execution Implementations
// ============================================================================
    ...
    if m == EA_DN {
        if s == SIZE_LONG {
            match cpu.state.micro.micro_step {
                ...
                3 => {
                    ...
                    let res = logic::execute_and(&mut cpu.state, imm, d, Size::Long);
            ...
            match cpu.state.micro.micro_step {
                ...
                1 => {
                    ...
                    let reg_d = (cpu.state.ir & 7) as usize;
                    let d = cpu.state.d[reg_d];
                    let res = logic::execute_and(&mut cpu.state, imm, d, size);
        ...
        if s == SIZE_LONG {
            if phase == 0 {
                ...
                let res = logic::execute_and(&mut cpu.state, imm, mem_val, Size::Long);
            ...
            if phase == 0 {
                ...
                let imm = cpu.state.micro.scratch[3];
                let res = logic::execute_and(&mut cpu.state, imm, mem_val, size);
                cpu.state.micro.scratch[1] = res;
                cpu.initiate_prefetch();
                cpu.state.micro.scratch[2] = 1;
                StepResult::StepCompleted
    ...
    if m == EA_DN {
        if s == SIZE_LONG {
            match cpu.state.micro.micro_step {
                ...
                3 => {
                    ...
                    let reg_d = (cpu.state.ir & 7) as usize;
                    let d = cpu.state.d[reg_d];
                    let res = logic::execute_or(&mut cpu.state, imm, d, Size::Long);
                    cpu.write_d_reg(reg_d, res, Size::Long);
                    cpu.initiate_prefetch();
                    cpu.state.micro.mark_standard_prefetch_retire();
                    StepResult::StepCompleted
            ...
            match cpu.state.micro.micro_step {
                ...
                1 => {
                    ...
                    let reg_d = (cpu.state.ir & 7) as usize;
                    let d = cpu.state.d[reg_d];
                    let res = logic::execute_or(&mut cpu.state, imm, d, size);
                    cpu.write_d_reg(reg_d, res, size);
                    cpu.initiate_prefetch();
                    cpu.state.micro.mark_standard_prefetch_retire();
                    StepResult::StepCompleted
        ...
        if s == SIZE_LONG {
            if phase == 0 {
                ...
                let imm = cpu.state.micro.scratch[3];
                let res = logic::execute_or(&mut cpu.state, imm, mem_val, Size::Long);
                cpu.state.micro.scratch[1] = res;
                cpu.initiate_prefetch();
                cpu.state.micro.scratch[2] = 1;
                StepResult::StepCompleted
            ...
            if phase == 0 {
                ...
                let imm = cpu.state.micro.scratch[3];
                let res = logic::execute_or(&mut cpu.state, imm, mem_val, size);
                cpu.state.micro.scratch[1] = res;
                cpu.initiate_prefetch();
                cpu.state.micro.scratch[2] = 1;
                StepResult::StepCompleted
...
#[inline(always)]
fn exec_eori(cpu: &mut Cpu, bus: &mut MemoryBus, s: u8, m: u8) -> StepResult {
    let size = size_from_const(s);
    ...
    if m == EA_DN {
        if s == SIZE_LONG {
            match cpu.state.micro.micro_step {
                ...
                3 => {
                    ...
                    let reg_d = (cpu.state.ir & 7) as usize;
                    let d = cpu.state.d[reg_d];
                    let res = logic::execute_eor(&mut cpu.state, imm, d, Size::Long);
                    cpu.write_d_reg(reg_d, res, Size::Long);
                    cpu.initiate_prefetch();
                    cpu.state.micro.mark_standard_prefetch_retire();
                    StepResult::StepCompleted
            ...
            match cpu.state.micro.micro_step {
                ...
                1 => {
                    ...
                    let reg_d = (cpu.state.ir & 7) as usize;
                    let d = cpu.state.d[reg_d];
                    let res = logic::execute_eor(&mut cpu.state, imm, d, size);
                    cpu.write_d_reg(reg_d, res, size);
                    cpu.initiate_prefetch();
                    cpu.state.micro.mark_standard_prefetch_retire();
                    StepResult::StepCompleted
        ...
        if s == SIZE_LONG {
            if phase == 0 {
                ...
                let imm = cpu.state.micro.scratch[3];
                let res = logic::execute_eor(&mut cpu.state, imm, mem_val, Size::Long);
                cpu.state.micro.scratch[1] = res;
                cpu.initiate_prefetch();
                cpu.state.micro.scratch[2] = 1;
                StepResult::StepCompleted
            ...
            if phase == 0 {
                ...
                let imm = cpu.state.micro.scratch[3];
                let res = logic::execute_eor(&mut cpu.state, imm, mem_val, size);
                cpu.state.micro.scratch[1] = res;
                cpu.initiate_prefetch();
                cpu.state.micro.scratch[2] = 1;
                StepResult::StepCompleted
