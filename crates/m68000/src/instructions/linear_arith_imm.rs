//! Cycle-exact linear M68000 immediate arithmetic instructions (ADDI, SUBI, CMPI)
//!
    ...
    if m == EA_DN {
        ...
        if s == SIZE_LONG {
            if phase == 0 {
                ...
                let res = arithmetic::execute_add(&mut cpu.state, imm, mem_val, Size::Long, true);
    ...
    if m == EA_DN {
        ...
        if s == SIZE_LONG {
            if phase == 0 {
                ...
                let imm = cpu.state.micro.scratch[3];
                let res = arithmetic::execute_sub(&mut cpu.state, imm, mem_val, Size::Long, true);
                cpu.state.micro.scratch[1] = res;
                cpu.initiate_prefetch();
                cpu.state.micro.scratch[2] = 1;
                StepResult::StepCompleted
...
#[inline(always)]
fn exec_cmpi(cpu: &mut Cpu, bus: &mut MemoryBus, s: u8, m: u8) -> StepResult {
    let size = size_from_const(s);
    ...
    if m == EA_DN {
        if s == SIZE_LONG {
            ...
            match cpu.state.micro.micro_step {
                ...
                3 => {
                    ...
                    let reg_d = (cpu.state.ir & 7) as usize;
                    let d = cpu.state.d[reg_d];
                    arithmetic::execute_cmp(&mut cpu.state, imm, d, Size::Long);
                    cpu.initiate_prefetch();
                    cpu.state.micro.mark_standard_prefetch_retire();
                    StepResult::StepCompleted
                }
            ...
            match cpu.state.micro.micro_step {
                ...
                1 => {
                    ...
                    let reg_d = (cpu.state.ir & 7) as usize;
                    let d = cpu.state.d[reg_d];
                    arithmetic::execute_cmp(&mut cpu.state, imm, d, size);
                    cpu.initiate_prefetch();
                    cpu.state.micro.mark_standard_prefetch_retire();
                    StepResult::StepCompleted
                }
        ...
        if s == SIZE_LONG {
            ...
            let imm = cpu.state.micro.scratch[3];
            arithmetic::execute_cmp(&mut cpu.state, imm, mem_val, Size::Long);
            cpu.initiate_prefetch();
            cpu.state.micro.mark_standard_prefetch_retire();
            StepResult::StepCompleted
        } else {
            ...
            let imm = cpu.state.micro.scratch[3];
            arithmetic::execute_cmp(&mut cpu.state, imm, mem_val, size);
            cpu.initiate_prefetch();
            cpu.state.micro.mark_standard_prefetch_retire();
            StepResult::StepCompleted
        }
