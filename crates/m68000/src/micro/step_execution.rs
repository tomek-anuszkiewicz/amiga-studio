...
impl Cpu {
    ...
    pub(crate) fn step_bus_read_byte(&mut self, bus: &mut MemoryBus) -> StepResult {
    ...
    pub(crate) fn step_bus_read_word(&mut self, bus: &mut MemoryBus) -> StepResult {
        let addr = self.state.micro.ea_addr;
        if (addr & 1) != 0 {
    ...
    pub(crate) fn step_bus_read_long_high(&mut self, bus: &mut MemoryBus) -> StepResult {
        let addr = self.state.micro.ea_addr;
        if (addr & 1) != 0 {
            return self.trigger_address_error_step(addr, true, false, bus);
        }
    ...
    pub(crate) fn step_bus_read_long_low(&mut self, bus: &mut MemoryBus) -> StepResult {
    ...
    pub(crate) fn step_bus_write_byte(&mut self, bus: &mut MemoryBus) -> StepResult {
        let addr = self.state.micro.ea_addr;
    ...
    pub(crate) fn step_bus_write_word(&mut self, bus: &mut MemoryBus) -> StepResult {
        let addr = self.state.micro.ea_addr;
        if (addr & 1) != 0 {
    ...
    pub(crate) fn step_bus_write_long_high(&mut self, bus: &mut MemoryBus) -> StepResult {
        let addr = self.state.micro.ea_addr;
        if (addr & 1) != 0 {
            return self.trigger_address_error_step(addr, false, false, bus);
        }
    ...
    pub(crate) fn step_bus_write_long_low(&mut self, bus: &mut MemoryBus) -> StepResult {
        let addr = self.state.micro.ea_addr.wrapping_add(2);
        if (addr & 1) != 0 {
            return self.trigger_address_error_step(addr, false, false, bus);
        }
    ...
    pub(crate) fn step_bus_write_word_and_retire(&mut self, bus: &mut MemoryBus) -> StepResult {
        let addr = self.state.micro.ea_addr;
        if (addr & 1) != 0 {
        ...
        let res = self.step_write_word_at(bus, addr, val);
        if res == StepResult::StepCompleted && self.state.micro.phase == CckPhase::Cck1 {
            return self.retire_scratch_prefetch();
        }
        res
    }

    pub(crate) fn step_bus_write_byte_and_retire(&mut self, bus: &mut MemoryBus) -> StepResult {
        let addr = self.state.micro.ea_addr;
        let val = (self.state.micro.write_buffer & 0xFF) as u8;
        let res = self.step_write_byte_at(bus, addr, val);
        if res == StepResult::StepCompleted && self.state.micro.phase == CckPhase::Cck1 {
    ...
    pub(crate) fn step_bus_write_long_low_and_retire(&mut self, bus: &mut MemoryBus) -> StepResult {
        let addr = self.state.micro.ea_addr.wrapping_add(2);
        if (addr & 1) != 0 {
            return self.trigger_address_error_step(addr, false, false, bus);
        }
    ...
    pub(crate) fn step_bus_write_long_high_and_retire(&mut self, bus: &mut MemoryBus) -> StepResult {
        let addr = self.state.micro.ea_addr;
        if (addr & 1) != 0 {
            return self.trigger_address_error_step(addr, false, false, bus);
        }
    ...
    pub(crate) fn step_bus_pop_stack(&mut self, bus: &mut MemoryBus) -> StepResult {
    ...
    pub(crate) fn step_bus_pop_stack_high(&mut self, bus: &mut MemoryBus) -> StepResult {
        let sp = self.state.read_a(7);
        if (sp & 1) != 0 {
            return self.trigger_address_error_step(sp, true, false, bus);
        }
    ...
    pub(crate) fn step_bus_pop_stack_low(&mut self, bus: &mut MemoryBus) -> StepResult {
        let sp = self.state.read_a(7);
        if (sp & 1) != 0 {
            return self.trigger_address_error_step(sp, true, false, bus);
        }
    ...
    pub(crate) fn step_bus_push_stack_high(&mut self, bus: &mut MemoryBus) -> StepResult {
    ...
    pub(crate) fn step_bus_push_stack_low(&mut self, bus: &mut MemoryBus) -> StepResult {
    ...
    pub(crate) fn step_bus_push_stack_low_and_retire(&mut self, bus: &mut MemoryBus) -> StepResult {
        let sp_low = self.state.read_a(7).wrapping_add(2);
        let val = (self.state.micro.write_buffer & 0xFFFF) as u16;
        let res = self.step_write_word_at(bus, sp_low, val);
        if res == StepResult::StepCompleted && self.state.micro.phase == CckPhase::Cck1 {
    ...
    pub(crate) fn step_fetch_extension(&mut self, bus: &mut MemoryBus) -> StepResult {
    ...
    pub(crate) fn step_bus_prefetch_to_scratch(&mut self, bus: &mut MemoryBus) -> StepResult {
        let addr = self.state.pc;
        let res = self.step_read_prog_word_at(bus, addr);
        if res == StepResult::StepCompleted && self.state.micro.phase == CckPhase::Cck1 {
    ...
    pub(crate) fn step_prefetch_next_opcode_and_retire(&mut self, bus: &mut MemoryBus) -> StepResult {
        let addr = self.state.pc;
        let res = self.step_read_prog_word_at(bus, addr);
        if res == StepResult::StepCompleted && self.state.micro.phase == CckPhase::Cck1 {
            let word = self.state.micro.last_read;
            return self.retire_standard(word);
        }
        res
    }

    pub(crate) fn step_bus_read_target_opcode(&mut self, bus: &mut MemoryBus) -> StepResult {
        let addr = self.state.micro.ea_addr;
        if (addr & 1) != 0 {
    ...
    pub(crate) fn step_prefetch_target_and_retire(&mut self, bus: &mut MemoryBus) -> StepResult {
        let addr = self.state.micro.ea_addr.wrapping_add(2);
        ...
        if res == StepResult::StepCompleted && self.state.micro.phase == CckPhase::Cck1 {
            ...
            return self.retire_target_refill(target, target_prefetch, new_ir);
        }
        res
    }
}
