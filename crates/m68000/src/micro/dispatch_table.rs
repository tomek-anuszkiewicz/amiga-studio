...
pub const fn build_opcode_descriptor_table() -> [OpcodeDescriptor; 65536] {
    ...
    // Migrated opcodes will be registered here phase-by-phase.
    // Opcodes with steps.is_empty() fall through to legacy DISPATCH_TABLE.
    ...
    while op <= 0x1FFF {
        ...
        if let Some(steps) = crate::instructions::move_b::decode_move_b_steps(src_mode, src_reg, dst_mode, dst_reg) {
            table[op] = OpcodeDescriptor {
                steps,
                reg_src: src_reg,
                reg_dst: dst_reg,
    ...
    // Phase 3: CMP and CMPA opcodes ($B000..=$BFFF)
    ...
    while op <= 0xBFFF {
        ...
        if size < 3 {
            if dir == 0 {
                ...
                // EOR Dn, <ea>
                if let Some(steps) = crate::instructions::eor::decode_eor_steps(size, mode, reg) {
                    table[op] = OpcodeDescriptor {
                        steps,
                        reg_src: reg_d,
                        reg_dst: reg,
            ...
            // CMPA: size == 3. dir == 0 is Word, dir == 1 is Long
            let is_long = dir != 0;
    ...
    // Phase 3: CMPI opcodes ($0C00..=$0CFF)
    ...
    table[0x003C] = OpcodeDescriptor { steps: &crate::instructions::ori::STEPS_ORI_CCR, reg_src: 0, reg_dst: 0 };
    table[0x007C] = OpcodeDescriptor { steps: &crate::instructions::ori::STEPS_ORI_SR, reg_src: 0, reg_dst: 0 };

    // Phase 4: ANDI opcodes ($0200..=$02FF)
    ...
    table[0x023C] = OpcodeDescriptor { steps: &crate::instructions::andi::STEPS_ANDI_CCR, reg_src: 0, reg_dst: 0 };
    table[0x027C] = OpcodeDescriptor { steps: &crate::instructions::andi::STEPS_ANDI_SR, reg_src: 0, reg_dst: 0 };

    // Phase 4: EORI opcodes ($0A00..=$0AFF)
    ...
    table[0x0A3C] = OpcodeDescriptor { steps: &crate::instructions::eori::STEPS_EORI_CCR, reg_src: 0, reg_dst: 0 };
    table[0x0A7C] = OpcodeDescriptor { steps: &crate::instructions::eori::STEPS_EORI_SR, reg_src: 0, reg_dst: 0 };
    ...
    // Phase 5: Shift and Rotate opcodes ($E000..=$EFFF)
    let mut op = 0xE000usize;
    while op <= 0xEFFF {
        let ir = op as u16;
        let size_bits = ((ir >> 6) & 3) as u8;
        if size_bits == 3 {
            // Memory shift/rotate (Word only, Count = 1, bit 11 must be 0)
            if ((ir >> 11) & 1) == 0 {
                ...
                let steps_opt = match (shift_type, dir) {
