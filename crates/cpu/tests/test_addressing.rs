...
fn test_addressing_modes_and_a7_byte_quirk() {
    ...
    state.set_a_long(7, 0x002000); // SP
...
fn test_nop_and_branch() {
    ...
    // NOP (0x4E71)
    cpu.state.ir = 0x4E71;
    cpu.state.prefetch = 0x6004; // BRA.S +4 at 0x1000
    ...
    assert_eq!(cpu.state.pc, 0x001006 + 4); // +4 because target prefetch refill fetched 2 words into IR & prefetch
}
...
fn test_instruction_unaligned_read_address_error() {
    ...
    assert_eq!(cpu.state.a_long(7), 0x004000 - 14);
