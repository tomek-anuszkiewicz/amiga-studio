...
/// Helper: Initializes CPU and Amiga PhysicalMemory with Chip RAM mapped at `$000000`.
fn setup_machine() -> (Cpu, PhysicalMemory) {
...
fn load_program(bus: &mut PhysicalMemory, start_addr: u32, code: &[u16]) {
...
fn execute_instructions(cpu: &mut Cpu, bus: &mut PhysicalMemory, count: usize) {
    for _ in 0..count {
...
fn test_program_subroutine_call_and_return() {
    ...
    cpu.state.set_ssp(initial_sp);
    cpu.state.set_usp(initial_sp);
...
fn test_program_trap_exception_and_rte() {
    ...
    cpu.state.set_ssp(ssp);
    cpu.state.set_usp(usp);
...
fn test_program_bcd_arithmetic() {
    ...
    let (mut cpu, mut bus) = setup_machine();
    load_program(&mut bus, 0x001000, &code);
    cpu.state.set_sr(cpu.state.sr() & !0x0015); // Ensure X, Z, C flags are 0
    cpu.set_pc_and_prime_prefetch(0x001000, &mut bus);
