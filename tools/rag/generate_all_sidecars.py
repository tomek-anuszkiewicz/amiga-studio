...
# Ensure tools/rag is in python path
...
MANUALS_ROOT = Path(r"D:\Programowanie\Amiga\Obsidian\Amiga\Reference")
...
def main():
    mgr = AssetsManager()
    unindexed = mgr.get_unindexed_images(MANUALS_ROOT.parent)
    print(f"Total unindexed images detected by AssetsManager: {len(unindexed)}")

    created_count = 0
    for item in unindexed:
        img_p = item["path"]
        book_dir = img_p.parents[1]  # Reference / <Book Name>
...
"""
        ...
        mgr.record_description(img_p, desc_text, root_dir=MANUALS_ROOT.parent)
        created_count += 1
    ...
    status_after = mgr.get_unindexed_images(MANUALS_ROOT.parent)
